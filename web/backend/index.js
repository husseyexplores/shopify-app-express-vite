...
import * as googleOauth from './oath/google.js'
...
app.get('/api/redirect', decodeStateMiddleware, (req, res) => {
  ...
  try {
    ...
    if (to === 'google') {
      res.redirect(`${googleOauth.OAuthUrl}&state=${state}`)
    } else {
...
// Google auth `/api/exoauth/google`
app.get(env.GOOGLE_OAUTH_REDIRECT_URI.pathname, googleOauth.callback)
